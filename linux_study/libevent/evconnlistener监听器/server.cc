...
{
	struct sockaddr_in sin;  //evthread_use_pthreads(); //enable threads 
	...
	struct evconnlistener *listener = evconnlistener_new_bind(base, listener_cb, base,  
	 ...
	 LEV_OPT_REUSEABLE | LEV_OPT_CLOSE_ON_FREE,10, (struct sockaddr*)&sin, sizeof(struct sockaddr_in) );
	...
	return 0;
                                                                                                          ...
                                                                                                          //这个arg应该不用我们传递，应该是libevent内部赋值
void listener_cb(struct evconnlistener *listener, evutil_socket_t fd, struct sockaddr *sock, int socklen, void *arg) {

	printf("accept a client %d \n",fd);  //一个新客户端连接上服务器了  //当此函数被调用时，libevent已经帮我们accept了这个客户端,该客户端的文件描述符为fd

	struct event_base *base = (struct event_base*)arg;

	struct bufferevent *bev = bufferevent_socket_new(base,fd,BEV_OPT_CLOSE_ON_FREE);  //为这个客户端分配一个bufferevent

...
//      return false; //解析数据到body中
    ...
    string strData((char*)(*curData), JsonSize);
    tutorial::CreateUDiskRequest udisk;
    if (!udisk.ParseFromString(strData)) {
        return false;
    }

    cout << "top_oid: "   << udisk.top_oid() << endl;
    cout << "oid: "       << udisk.oid() << endl;
    cout << "extern_id: " << udisk.extern_id() << endl;
    cout << "name: "      << udisk.name() << endl;
    cout << "size: "      << udisk.size() << endl;
    cout << "owner: "     << udisk.owner() << endl;
    cout << "disk_type: " << udisk.disk_type() << endl;

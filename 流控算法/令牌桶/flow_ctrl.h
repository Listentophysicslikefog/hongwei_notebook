...
class TokenBucket {
 public:
  static const int64_t kUnlimitedRate = 1LL << 50;
  TokenBucket(uint64_t capacity, uint64_t rate)
      : tokens_(capacity),
        cycles_(XGetTimeCycle()),
        capacity_(capacity),
        rate_(rate) {}
      ...
      tokens_[i] = std::min(
          ...
          tokens_[i] +
              static_cast<uint32_t>(
                  XTimeStampDiff(now_time, timestamp_) * rate_[i] + 0.5) *
  ...
  bool grant(T* token_size) {
    ...
    for (i = 0; i < size; ++i) {
      ...
      if (UNLIKELY(token_size[i] > capacity_[i])) {
        XLOG_INFO << "token is larger than capacity, token=" << token_size[i]
...
class LeakyBucket {
 public:
  ...
  bool grant(uint64_t size) {
    ...
    uint64_t pay_off =
        static_cast<uint64_t>(XTimeCycleDiff(now_cycles, cycles_) * rate_);

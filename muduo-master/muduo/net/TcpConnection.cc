...
{
  channel_->setReadCallback( //通道可读事件到来的时候，回调tcpconnection::handleRead(),_1是事件发生时间
      std::bind(&TcpConnection::handleRead, this, _1));   //handleRead中会调用messageCallback_
...
void TcpConnection::connectEstablished()  //这里会关注可读事件，并且回调connectionCallback_，
{
  loop_->assertInLoopThread();
  ...
  channel_->enableReading();    //一旦连接成功就关注它的可读事件，tcpconnection所对应的通道加入到Poller中关注
//回调conn->setConnectionCallback(connectionCallback_)设置的回调函数

...
chunk  ip  set   usecount

class SourceloadBalance{
    void IncChunkUseCount(uint32_t chunk_id, uint32_t set_id);
    uint32_t GetChunkUseCount(uint32_t chunk_id, uint32_t set_id);
    void AddChunkInfo(uint32_t chunk_id, uint32_t set_id);
    void IncHostUseCount(const std::string &host_ip, uint32_t set_id);
    uint32_t GetHostUseCount(const std::string &host_ip, uint32_t set_id);
...
pc_list: ip chunkid set_id  ip chunkid set_id  ip chunkid set_id  ip chunkid set_id  ip chunkid set_id

map[uint32]map[pair<ip,chunk_id>]

...
namespace apollo
{
  namespace cyber
  {
    namespace logger
    {
      ...
      void AsyncLogger::Write(bool force_flush, const std::chrono::system_clock::time_point &timestamp, const char *message,
                              size_t message_len)
      {
        ...
        if (message_len > 0)
        {
          ...
          active_buf_->emplace_back(std::chrono::system_clock::to_time_t(timestamp), std::move(msg_str),
                                    log_level_map.at(message[0]));
          flag_.clear(std::memory_order_release);
        }

        if (force_flush && std::chrono::system_clock::to_time_t(timestamp) == 0 && message && message_len == 0)
        {
          Stop();
        }
      }
      ...
      void AsyncLogger::FlushBuffer(const std::unique_ptr<std::deque<Msg>> &buffer)
      {
        ...
        while (!buffer->empty())
        {
          ...
          module_logger_map_.find(module_name)
              ->second->Write(force_flush, std::chrono::system_clock::from_time_t(msg.ts), msg.message.data(),
